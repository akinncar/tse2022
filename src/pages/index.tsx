...
export default function Home() {
  ...
  async function loadResults() {
    ...
    setLastDt(response.dt ? response.dt.split("/").reverse().join("-") : "-");
    setTotalAccurated(response.psi);
  }
    ...
    <div className="container py-4">
      ...
      <header>
        ...
        <p className="text-secondary">
          Última atualização:{" "}
          <time dateTime={lastDt + " " + lastHt}>{lastHt}</time>
        </p>
      </header>
          ...
          <div className="row w-100 card-body d-flex flex-row align-items-center">
            <div className="col">
              ...
              <progress
                className="w-100"
                max="100"
                value={Number(totalAccurated.replace(",", "."))}
              >
                {totalAccurated}%
              </progress>
            </div>
            ...
            <tbody>
              {candList.map((cand, index) => (
                <tr key={cand.nm}>
                  ...
                  <td>
                    <span className="votes-percentage-textval">
                      {cand.pvap}%
                    </span>
                    <progress
                      style={{ display: "block", width: "100%" }}
                      className="votes-percentage-progress"
                      max="100"
                      value={Number(cand.pvap.replace(",", "."))}
                    >
                      {cand.pvap}%
                    </progress>
                  </td>
                  <td>
                    {Number(cand.vap).toLocaleString(undefined, {
